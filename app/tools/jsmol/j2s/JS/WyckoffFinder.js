...
Clazz.load(["java.util.Hashtable", "JU.P3", "$.V3"], "JS.WyckoffFinder", ["JU.JSJSONParser", "$.M4", "$.Measure", "$.P4", "$.PT", "$.Rdr", "$.SB", "JS.SymmetryOperation", "JV.FileManager"], function(){
...
JS.WyckoffFinder.getList(coords, letter, sbc, 0);
if (i > 0 && this.ncent > 0) {
var tempOp =  new JU.M4();
for (var j = 0; j < this.ncent; j++) {
this.addCentering(coords, this.centerings[j], tempOp, sbc);
}
}return sbc.toString();
}}
break;
...
function(centering, tempOp, sb){
tempOp.setM4(this.op);
tempOp.add(centering);
sb.appendC(' ');
var s = "," + JS.SymmetryOperation.getXYZFromMatrix(tempOp, false, true, true) + ",";
s = JU.PT.rep(s, ",,", ",0,");
s = JU.PT.rep(s, ",+", ",");
...
;//5.0.1-v2 Mon Feb 26 09:31:58 CST 2024
